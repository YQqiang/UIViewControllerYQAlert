...
#import "SGLinkLabel.h"
#import <CoreText/CoreText.h>
#define WXLABEL_IMAGE_NAME @"imageName"
@interface SGLinkLabel ()
...
- (void)_createAttributeText {
    ...
    //自定义当前超链接文本颜色
    if ([self.sgLinkLabelDelegate respondsToSelector:@selector(linkColorWithSGLinkLabel:)]) {
        self.linkColor = [self.sgLinkLabelDelegate linkColorWithSGLinkLabel:self];
    }
    
    //自定义当前超链接文本颜色
    if ([self.sgLinkLabelDelegate respondsToSelector:@selector(passColorWithSGLinkLabel:)]) {
        self.passColor = [self.sgLinkLabelDelegate passColorWithSGLinkLabel:self];
    }
    if (self.text == nil) {
        return;
    }
    ...
    //----------------------设置链接文本的颜色-------------------
    //判断当前链接文本表达式是否实现
    if ([self.sgLinkLabelDelegate respondsToSelector:@selector(contentsOfRegexStringWithSGLinkLabel:)] && [self.sgLinkLabelDelegate contentsOfRegexStringWithSGLinkLabel:self] != nil)
    {
        //获取所有的链接文本
        ...
        for (NSValue *value in ranges) {
            ...
            if ([self.sgLinkLabelDelegate respondsToSelector: @selector(linkUnderlineStyleWithSGLinkLabel:)]) {
                CTUnderlineStyle underLineStyle = [self.sgLinkLabelDelegate linkUnderlineStyleWithSGLinkLabel:self];
                [_attrString addAttribute:(id)kCTUnderlineStyleAttributeName value:(id)[NSNumber numberWithInt:underLineStyle] range:range];
            }
        }
...
- (NSArray *)imagesOfRegexStrArray
{
    ...
    if ([self.sgLinkLabelDelegate respondsToSelector:@selector(imagesOfRegexStringWithSGLinkLabel:)]) {
        regex = [self.sgLinkLabelDelegate imagesOfRegexStringWithSGLinkLabel:self];
    }
    
    //通过正则表达式查找出匹配的字符串
    NSArray *matchArray = [self componentsMatchedByRegex:regex contentText:self.text];
    //<image url = 'wxhl.png'>
    return matchArray;
}
...
- (NSArray *)contentsOfRegexStrArray
{
    ...
    NSString *regex = [self.sgLinkLabelDelegate contentsOfRegexStringWithSGLinkLabel:self];
    
    //通过正则表达式查找出匹配的字符串
    NSArray *matchArray = [self componentsMatchedByRegex:regex contentText:[self.attrString string]];
...
- (NSArray *)componentsMatchedByRegex:(NSString *)regex contentText:(NSString *)content {
    ...
    NSArray *matches = [regular matchesInString:content options:NSMatchingReportProgress range:NSMakeRange(0, content.length)];
    NSMutableArray *results = [NSMutableArray array];
    for (NSTextCheckingResult *result in matches) {
        [results addObject: [content substringWithRange:result.range]];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (range.length == 0) {
        ...
        //判断当前代理方法是否实现
        if ([self.sgLinkLabelDelegate respondsToSelector:@selector(toucheEndSGLinkLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.sgLinkLabelDelegate toucheEndSGLinkLabel:self withContext:context];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        //判断当前代理方法是否实现
        if ([self.sgLinkLabelDelegate respondsToSelector:@selector(toucheBenginSGLinkLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.sgLinkLabelDelegate toucheBenginSGLinkLabel:self withContext:context];
        }
    }
...
- (NSRange)touchInLabelText:(CGPoint)point
{
    ...
    int indexLine = point.y / lineHeight - 0.5;
...
{
    SGLinkLabel *wxLabel = [[SGLinkLabel alloc] initWithFrame:CGRectMake(0, 0, width, 900)];
